...
const ChildThumbnails = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        // Fetch genres
        const genresResponse = await axios.get("http://192.168.0.11:8800/genres");
        setGenres(genresResponse.data);

        // Fetch child-friendly movies
        const childMoviesResponse = await axios.get("http://192.168.0.11:8800/getChildFriendlyMovies");
