...
const ProfilesList = () => {
  ...
  const handlePinValidation = async (profileId) => {
  ...
  // Inside handleAddProfile function
const handleAddProfile = () => {
  if (profiles.length < 4) {
    // Allow adding profile
    navigate("/add-profile");
  } else {
    // Show error message when maximum profiles are reached
              ...
              <li key={index}>
                <div className={styles.profileinfo}>
                  ...
                  {profile.profilePhotoPath ? (
                    ...
                    <img
                onClick={() => handlePinValidation(profile.id)}
                src={
